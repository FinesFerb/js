...
interface IPage {
  ...
  header: {
    ...
    num_progres: HTMLElement | null;
    line_active: HTMLElement | null;
  }
  comment: {
    coverComment: HTMLElement | null;
    commentInfo: HTMLElement | null;
    text_comment: HTMLElement | null;
    count_day_input: HTMLElement | null;
  }
  popup: {
    index: HTMLElement | null;
    inputHidden: HTMLElement | null;
    inputTitle: HTMLElement | null; 
    inputTarget: HTMLElement | null;
  }
}
let habbits: IHabbit[] = [];
const HABBIT_KEY = "HABBIT_KEY";
let globalActiveHabbitId: number
...
// itils
function loadData() {
  const habbitsString: string | null = localStorage.getItem(HABBIT_KEY)
...
// render
function rerenderMenu(activeHabbit: IHabbit) {
...
function rerenderHeader(activeHabbit: IHabbit) {
...
function rerenderComment(activeHabbit: IHabbit) {   
...
function rerender (activeHabbitId: number) { 
...
function addDay(e: Event) {
  ...
  const comment = commentToString as string
  ...
  (e.target as any).value_comment.value = ''
  rerender(globalActiveHabbitId)
  saveData()
}

// delet day
function deletDay(index: number) {
  habbits = habbits.map(habbit => { 
...
//work popup
function setIcon(context: any, icon: string) {
  if (page.popup.inputHidden) {
    (page.popup.inputHidden as any).value = icon
  }
